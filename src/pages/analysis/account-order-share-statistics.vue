<template>
  <x-page title="订单占比分析" breadcrumb="auto">
    <el-card body-style="padding: 10px;position: relative;" class="el-card-mini no-box-shadow no-border-radius box-card">
      ...
      <el-row>
        <el-col :span="12">
          <e-charts v-if="orderCharts" :options="orderCharts"/>
        </el-col>
        <el-col :span="12">
          <e-charts v-if="depositCharts" :options="depositCharts"/>
        </el-col>
      </el-row>
      <div class="show-table" v-show="showTable">
        <el-table
          ...
          <el-table-column prop="depositAmount" label="押金合计">
            <span slot-scope="scope">{{scope.row.depositAmount | currency}}</span>
          </el-table-column>
        </el-table>
      </div>
    </el-card>
      ...
      try {
        ...
        if (res && res.aoAnalysisDatas) {
          res.aoAnalysisDatas.forEach(item => {
            orders.push({name: item.key, value: item.orderCount})
            deposits.push({name: item.key, value: item.depositAmount})
          })
        }
        this.orders = orders
        this.deposits = deposits
        this.tableData = res.aoAnalysisDatas
      } catch (e) {
