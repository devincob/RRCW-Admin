  ...
  <x-page breadcrumb="auto" style="padding:24px 12px;" title="人人财务交易管理系统">
    ...
    <el-row>
      <el-col :span="24" style="padding: 12px;">
        <el-card class="my-card-table el-card-mini no-box-shadow no-border-radius mt-lg">
          <div slot="header"><span style="font-size: 14px;" class="text-bold">待处理开户订单</span></div>
          <el-table
            size="mini"
            :data="tableData"
            style="width: 100%">
            <el-table-column prop="orderNo" label="订单号" width="200">
              <div slot-scope="scope">
                <router-link :to="(scope.row.workflowId === 0 ? '/order/account-order-create?orderid=' : '/order/account-order-details?orderid=') + scope.row.orderId" target="_blank">{{scope.row.orderNo}}</router-link>
            ...
            <el-table-column prop="totalAmount" label="交易费">
      ...
      <el-col :span="24" style="padding: 0 12px;">
        <el-card class="my-card-table el-card-mini no-box-shadow no-border-radius mt-lg">
          <div slot="header"><span style="font-size: 14px;" class="text-bold">待处理开票订单</span></div>
          <el-table
            size="mini"
            :data="tableData2"
            style="width: 100%">
            <el-table-column prop="orderNo" label="订单号" width="200">
              <div slot-scope="scope">
                <router-link :to="(scope.row.workflowId === 0 ? '/order/invoice-order-create?orderid=' : '/order/invoice-order-details?orderid=') + scope.row.orderId" target="_blank">{{scope.row.orderNo}}</router-link>
                <el-tag size="mini" v-if="scope.row.isPriority === 'Y'" type="danger">急</el-tag>
              </div>
            </el-table-column>
            <el-table-column prop="customerName" label="客户姓名"/>
