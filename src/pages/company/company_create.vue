<template>
  ...
  >
    <el-card
      ...
      style="max-width:800px;min-width:600px;min-height:1000px"
    >
      ...
      >
        ...
        >
          <el-input
            ...
            disabled
            :placeholder="ruleForm.companyId ? ruleForm.companyId : '系统自动生成'"
          ></el-input>
        </el-form-item>
        <el-form-item
          label="站点名称"
        ...
        >
          <el-input
            v-model="ruleForm.accountBankAccount"
        ...
        >
          <el-input
            type="number"
            v-model.number="ruleForm.serviceFeeDiscount"
            placeholder="请输入站点开票服务费费率折扣"
          ></el-input>
        </el-form-item>
        <el-form-item
          label="税源地"
        ...
        >
          <el-upload
            class="avatar-uploader upload-mini"
          ...
          <!-- <el-upload
            ref="uploadPhoto"
            :class="{'upload-limit-max': imageUrl}"
            :action="$$main.getUrl('/Common/ImageUpload')"
            :limit="1"
  ...
  methods: {
    ...
    loadInfo(companyId) {
      this.$$main.companyDetail({ companyId }).then(company => {
        ...
        this.imageUrls = []
        this.imageUrls.push({ url: Config.getImageUrlPath() + this.imageUrl })
    ...
    handleRemove(file, fileList) {
      if (fileList && fileList.length <= 0) {
        this.ruleForm.licenseUrl = this.imageUrl = ''
        this.imageUrls = []
      } else {
        this.ruleForm.licenseUrl = this.imageUrl = fileList[0]
        this.imageUrls = []
        this.imageUrls.push({ url: Config.getImageUrlPath() + this.imageUrl })
      }
    },
    handleError(e) {
      this.$message.error((e && e.message) || '系统繁忙，请稍后重试')
    },
    onPageShow() {
      this.ruleForm = {
        'companyName': null,
