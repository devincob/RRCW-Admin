<template>
  <div style="display: -webkit-box;">
    <div class="functional-select-wrapper province-wrapper">
      <label class="display-container multiple-select-container clearfix" v-bind:class="[(showProvince)?'single-selected-focus':'', customClass]" v-on:click.stop="multipleFocus()" style="width: 95%;">
        <span v-show="!selectedProvinceItem">全国</span>
        <span>{{ selectedProvinceItem.provinceName }}</span>
        <i class="drop" v-bind:class="showProvince ? 'drop-up': ''"><el-icon class="el-icon-arrow-down" style="color: #80848f;"></el-icon></i>
      </label>
      <div class="options-container" v-show="showProvince" style="width: 95%">
        <div class="search-container">
          <input style="width: 100%;" ref="searchInput" placeholder="搜索" class="search-input" v-model="searchProvince" v-on:keyup="multipleSearch($event)"/>
        </div>
        <ul class="options-ul-list">
          <li v-show="displayProvinceList.length === 0">无数据</li>
          <li v-for="item in displayProvinceList" :key="item.distictId" v-on:click.stop.prevent="multipleSelect(item)" v-bind:class="item.distictId === selectedProvinceItem.distictId ? 'selected' : ''">{{ item.provinceName }}</li>
        </ul>
      </div>
    </div>
    <div class="functional-select-wrapper city-wrapper">
      <label class="display-container multiple-select-container clearfix" v-bind:class="[(showCity)?'single-selected-focus':'', customClass]" v-on:click.stop="cityMultipleFocus()">
        <span v-show="!selectedCityItem">全部城市</span>
        <span>{{ selectedCityItem.districtName }}</span>
        <i class="drop" v-bind:class="showCity ? 'drop-up': ''"><el-icon class="el-icon-arrow-down" style="color: #80848f;"></el-icon></i>
      </label>
      <div class="options-container" v-show="showCity">
        <div class="search-container">
          <input style="width: 100%;" ref="citySearchInput" placeholder="搜索" class="search-input" v-model="searchCity" v-on:keyup="cityMultipleSearch($event)"/>
        </div>
        <ul class="options-ul-list">
          <li v-show="displayCityList.length === 0">无数据</li>
          <li v-for="item in displayCityList" :key="item.distictId" v-on:click.stop.prevent="cityMultipleSelect(item)" v-bind:class="{'selected' : item.distictId === selectedCityItem.distictId, 'disable' : showOpen && item.isOpen === 'N'}">{{ item.districtName }}{{ item.isOpen === 'N' && showOpen ?  '(未开通)' : '' }}</li>
        </ul>
      </div>
    </div>
  </div>
