...
class CacheManager {
  ...
  static setValue(key, value, expired) {
    Utils.Storage.set(key, value, expired)
  }
}
...
class Model {
   ...
   */
  getParam() {
    const state = Utils.Storage.get('VUEX_STATE')
  ...
  _getOptions(options) {
    ...
    return {
      ...
      data: JSON.stringify(Utils.extend(this.getParam(), options || {})),
      ...
      success: function (response) {
        ...
        if (!response.isSuccess) {
          if (response.code === 'LG301' || response.code === 'LG302' || response.code === 'LG303') {
            ...
            jQuery(document).trigger('login_expired', new Error(response.message))
            return
          }

          if (response.code === 'BCS15') {
            // Native.trigger('user_access_disable');
            jQuery(document).trigger('user_access_disable', new Error(response.message))
            return
          }
